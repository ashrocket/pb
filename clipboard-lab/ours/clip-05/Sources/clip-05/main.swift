...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private var deferredOCRService: OCRService?
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        do {
            ...
            poller.onNewRecord = { [weak historyWindow] _ in
                historyWindow?.reloadData(reset: true)
            }
            poller.onRecordUpdated = { [weak historyWindow] id in
                historyWindow?.refreshRecord(id: id)
            }
            poller.start()
            self.poller = poller
    ...
    private func finishDeferredStartup() {
        guard let configController else {
            return
        }

        deferredOCRService = OCRService()
        poller?.ocrService = deferredOCRService
    ...
    private func copy(_ record: ClipRecord) {
        ...
        switch record.kind {
        case .text, .url:
            if let text = record.displayText {
