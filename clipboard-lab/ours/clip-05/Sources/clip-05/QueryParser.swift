...
enum QueryParser {
    ...
    static func parse(_ raw: String?) throws -> ParsedQuery {
        ...
        guard !trimmed.isEmpty else {
            return ParsedQuery(clauses: [], bindings: [], textTerms: [])
        }
        ...
        func appendTextTerms(_ token: String) {
            textTerms.append(contentsOf: SearchIndex.queryTerms(for: token))
        }
        ...
        for token in tokenize(trimmed) {
            ...
            appendTextTerms(token)
        }

        return ParsedQuery(clauses: clauses, bindings: bindings, textTerms: textTerms)
    }
