        ...
        if let text = item.string(forType: .string) {
            ...
            if let detectedURL = URLDetector.detectStandaloneURL(in: normalized) {
                return ClipDraft(
                    ...
                    sourceApp: sourceApp,
                    kind: .url,
                    searchableText: searchableText(for: .url, sourceApp: sourceApp, primaryValue: detectedURL.text, extras: ["url", detectedURL.url.absoluteString]),
                    textValue: detectedURL.text,
                    payload: nil,
