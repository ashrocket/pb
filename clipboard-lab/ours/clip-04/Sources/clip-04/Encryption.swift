...
final class EncryptionManager {
    ...
    private let payloadInfo = Data("clip-04 payload encryption".utf8)
    private let passphraseAccount = "payload-passphrase"
    private let saltAccount = "payload-salt"

    init(keychain: KeychainValueStore = SecurityKeychainStore(service: "local.clip04.history")) {
    ...
    func decrypt(_ ciphertext: Data) throws -> Data {
        ...
        let combined = ciphertext.dropFirst()
        let sealedBox = try AES.GCM.SealedBox(combined: combined)
        return try AES.GCM.open(sealedBox, using: derivedKey())
    }

    private func derivedKey() throws -> SymmetricKey {
