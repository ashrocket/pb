...
struct SearchQuery {
    ...
    static func parse(_ rawValue: String?) -> SearchQuery {
        let trimmed = rawValue?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        guard !trimmed.isEmpty else {
            return SearchQuery(imagesOnly: false, text: nil)
        }
        ...
        if lowered.hasPrefix("image:") {
            ...
            return SearchQuery(imagesOnly: true, text: remainder.isEmpty ? nil : remainder)
        }

        return SearchQuery(imagesOnly: false, text: trimmed)
    }
}
...
final class Store {
    ...
    init(maxEntries: Int = 200, fileManager: FileManager = .default) throws {
        ...
        let directoryURL = supportRoot.appendingPathComponent("clip-02", isDirectory: true)
    ...
    func fetchEntries(search: String?) -> [ClipRecord] {
        let query = SearchQuery.parse(search)
        let textSearch = query.text?.trimmingCharacters(in: .whitespacesAndNewlines)
        let hasTextSearch = !(textSearch?.isEmpty ?? true)
        ...
        if hasTextSearch {
            clauses.append("LOWER(searchable_text) LIKE LOWER(?)")
        }
        ...
        do {
            return try withStatement(sql) { statement in
                var index: Int32 = 1
                if hasTextSearch, let textSearch {
                    sqlite3_bind_text(statement, index, "%\(textSearch)%", -1, sqliteTransient)
                    index += 1
                }
