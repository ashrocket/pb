...
struct ClipRecord {
    ...
    var detailText: String {
        ...
        if kind == .image, let payload {
            parts.append(ByteCountFormatter.string(fromByteCount: Int64(payload.count), countStyle: .file))
        }
...
final class Store {
    ...
    init(maxEntries: Int = 200, fileManager: FileManager = .default) throws {
        ...
        let directoryURL = supportRoot.appendingPathComponent("clip-01", isDirectory: true)
    ...
    func fetchEntries(search: String?) -> [ClipRecord] {
        ...
        let sql: String
        let bindSearch = !trimmedSearch.isEmpty

        if bindSearch {
            sql = """
            SELECT id, created_at, source_app, kind, searchable_text, text_value, payload
            FROM history
        ...
        do {
            let records = try withStatement(sql) { statement -> [ClipRecord] in
                var index: Int32 = 1
                if bindSearch {
    ...
    func save(_ draft: ClipDraft) throws -> ClipRecord? {
        ...
        try withStatement("""
        INSERT INTO history (id, created_at, source_app, kind, searchable_text, text_value, payload)
        VALUES (?, ?, ?, ?, ?, ?, ?);
        """) { statement in
