...
final class ClipboardPoller {
    ...
    private func makeDraft(from item: NSPasteboardItem, sourceApp: String?) -> ClipDraft? {
        ...
        if let text = item.string(forType: .string) {
            ...
            if let urlString = normalizedURL(from: normalized) {
                return ClipDraft(
                    createdAt: capturedAt,
                    sourceApp: sourceApp,
                    kind: .url,
                    searchableText: searchableText(for: .url, sourceApp: sourceApp, primaryValue: urlString, extras: ["url"]),
                    textValue: urlString,
                    payload: nil,
                    payloadBytes: urlString.lengthOfBytes(using: .utf8),
                    imageWidth: nil,
                    imageHeight: nil
                )
            }
            ...
            return ClipDraft(
                ...
                sourceApp: sourceApp,
                kind: .text,
                searchableText: searchableText(for: .text, sourceApp: sourceApp, primaryValue: normalized, extras: []),
        ...
        return ClipDraft(
            ...
            searchableText: searchableText(
                ...
                sourceApp: sourceApp,
                primaryValue: "image",
                extras: ["png", dimensions, "\(encoded.width)", "\(encoded.height)", byteLabel]
            ),
            textValue: nil,
    ...
    private func searchableText(for kind: ClipKind, sourceApp: String?, primaryValue: String, extras: [String]) -> String {
        let values: [String?] = [sourceApp, kind.rawValue, primaryValue] + extras.map { Optional($0) }
        return values
    ...
    private func normalizedURL(from text: String) -> String? {
        ...
        switch scheme {
        case "http", "https", "mailto", "ftp", "file":
            return text
        default:
            return nil
        }
